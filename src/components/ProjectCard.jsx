...
export default function ProjectCard({ project, index, className }) {
  ...
  // Get project hero image for prefetching
  const getProjectImage = (slug) => {
    switch (slug) {
      case 'groupe-adp':
        return '/assets/projects/groupe-adp/adp-logo.png';
      case 'abercrombie':
        return '/assets/projects/abercrombie/abercrombie-logo.png';
      case 'civic-vote':
  ...
  const renderPlaceholder = (proj) => {
    ...
    if (proj.slug === 'groupe-adp') {
      return (
        <div className="absolute inset-0 bg-[#0A2A5C] flex items-center justify-center overflow-hidden">
          <img 
    ...
    if (proj.slug === 'abercrombie') {
      return (
        <div className="absolute inset-0 bg-[#EAEAEA] flex items-center justify-center overflow-hidden">
          <img 
    ...
    if (proj.slug === 'civic-vote') {
      return (
        <div className="absolute inset-0 bg-[#8D4FE7] flex items-center justify-center overflow-hidden">
          <img 
    ...
    if (proj.slug === 'okane') {
      return (
        <div className="absolute inset-0 bg-[#5DB075] flex items-center justify-center overflow-hidden">
          <img 
    ...
    switch (type) {
      case 'grid-lines':
        return (
          <div className={`absolute inset-0 ${bgColor} flex flex-col justify-between p-8 overflow-hidden`}>
            {/* Airport lines grid */}
  ...
  const projectTitle = project.id === "02" 
    ? "Abercrombie & Fitch" 
    : project.id === "01" 
      ? "Groupe ADP" 
      : project.id === "03" 
  ...
  return (
    ...
    >
      {/* Entire Card wrapped in accessible PrefetchLink */}
      <PrefetchLink
        to={project.link}
        prefetchImage={getProjectImage(project.slug)}
        aria-label={`${projectTitle} — ${t.workSection.viewBtn}`}
        className="block w-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet focus-visible:ring-offset-4 rounded-xl cursor-none interactive-hover"
      >
        {/* Project Card Visual Preview */}
        <div className="relative w-full overflow-hidden aspect-[16/10] mb-5 border border-border-light rounded-xl bg-bg-light transition-all duration-500 group-hover:border-violet/60 group-hover:shadow-[0_8px_30px_rgba(238,184,249,0.16)]">
          {/* Render geometric customized template */}
          {renderPlaceholder(project)}
          
          {/* Transparent visual overlay frame that activates on hover */}
          <div className="absolute inset-0 bg-violet/90 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-400 ease-out flex flex-col justify-between p-6 sm:p-8 md:p-10 select-none">
            <div className="flex justify-between items-start">
              <span className="text-[10px] font-mono font-bold tracking-widest text-charcoal/70 uppercase">
                / {t.workSection.caseStudy}
              </span>
              <span className="text-[10px] font-mono px-3 py-1 border border-charcoal/20 text-charcoal rounded-full bg-charcoal/5 font-bold">
                {project.year}
              </span>
            </div>
            ...
            <div className="flex justify-between items-end">
              <div>
                <span className="text-[9px] uppercase tracking-widest font-mono font-extrabold text-charcoal/60">
                  {pContent.category}
                </span>
                <h4 className="text-2xl md:text-3xl font-syne font-black text-charcoal mt-1 uppercase leading-none">
                  {project.id} &bull; {projectTitle}
                </h4>
              </div>

              <div className="h-10 w-10 rounded-full border border-charcoal/30 flex items-center justify-center bg-charcoal/5 text-charcoal transition-transform duration-300 group-hover:scale-105">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
          <div className="flex gap-4 md:gap-5">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-baseline gap-2">
                <span className="font-syne text-lg md:text-xl font-bold text-charcoal group-hover:text-violet transition-colors duration-300 uppercase">
                  {projectTitle}
                </span>
          ...
          <div className="inline-flex items-center gap-1.5 text-xs font-mono font-bold tracking-widest uppercase text-charcoal-muted group-hover:text-violet transition-colors duration-300 mt-2 sm:mt-1 self-start sm:self-auto">
